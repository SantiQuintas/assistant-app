...
class ControladorMouse:
    ...
    def procesar_clicks(self, gesto_detectado):
        if gesto_detectado and not self.click_mantenido:
            pyautogui.mouseDown()
            self.click_mantenido = True
            print("¡Click Físico Ejecutado!")
        elif not gesto_detectado and self.click_mantenido:
            pyautogui.mouseUp()
            self.click_mantenido=False
    ...
    def mover_mouse(self, x_cam, y_cam, ancho_cam, alto_cam, margen):
        ...
        y_pantalla = np.interp(y_cam, (margen, alto_cam-margen), (0, self.alto_pantalla))
